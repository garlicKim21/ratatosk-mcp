...
CNCF 프로젝트 76개를 지켜보며 릴리스 노트 하나하나를 유형이 정해진 팩트로
바꿉니다. 보안 수정, 호환성이 깨지는 변경(breaking change), 기능 제거,
지원 중단, 기본값 변경 같은 것들이고, 팩트마다 영향을 받는 식별자까지
짚습니다. 단순 버그 수정과 홍보 문구는 걸러냅니다. 남는 것은 운영자가
조치할 내용입니다.
...
함께 씁니다 — 폴링이나 CI 워크로드라면 설치형으로 쓰세요. Claude Code CLI
기준:
...
`0.6.2`이 현재 릴리스이고, 새 릴리스를 계속 따라가려면 `latest`를 쓰세요.
...
> 것들입니다. 각 팩트에는 릴리스 노트 원문 인용이 근거로 붙습니다.
...
도구들이 쓰는 용어 둘을 먼저 정의합니다. **팩트(fact)**는 공식 릴리스
노트에서 뽑아낸 변경 하나입니다. 보안 수정, 기능 제거, 지원 중단, 기본값
변경 같은 것들이고, 팩트마다 그 변경이 건드리는 정확한 식별자(CVE id,
플래그, CRD, 설정 필드)와 근거가 되는 노트 원문 인용이 붙습니다. 모든
팩트에는 `info`부터 `critical`까지의 **심각도(severity)**가 있습니다.

| 도구 | 하는 일 |
|---|---|
| `check_stack` | 지금 실행 중인 컴포넌트 버전을 받아 업그레이드 경로의 팩트를 돌려줍니다 — critical/high는 나머지와 분리하고, 어드바이저리당 한 항목씩, 각각 인용과 id를 붙여 돌려줍니다. 비교는 서버 프로세스(설치형이라면 내 프로세스) 안에서 일어납니다([실행 중인 컴포넌트 버전의 처리 방식](#실행-중인-컴포넌트-버전의-처리-방식)) |
| `list_facts` | 증분 팩트 피드, 분석이 오래된 순. 프로젝트·타입·심각도로 거르고 `since` 커서로 페이지를 넘겨 로컬 사본을 동기화합니다 |
...
원칙입니다. 통제 밖의 경계 하나: CDN 계층(Cloudflare)의 접속 메타데이터는
Cloudflare 자체 정책을 따릅니다. 그 구간이 요건에 맞지 않으면 설치형을
...
설명합니다. API 키는 없고, IP당 분당 60회 제한이 있습니다.
