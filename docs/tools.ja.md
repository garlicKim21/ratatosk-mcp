...
7. **マージ規則が 2 つ、どちらも実測レスポンスに現れています。** ブランチを
   ...
   **他のすべてのリリース**が（返却ウィンドウの内外を問わず）
   ...
   採ります。
8. **比較するのは自分のリリースラインだけです。** 1 つのリポジトリが同じタグ体系で
   複数の製品・チャネルを出すことがあります。containerd はランタイムの `v2.2.5` の
   隣に `api/v1.11.1`（Go モジュール）を、Flatcar は `lts-4081.3.9` と
   `stable-4593.2.4` を並行して、OpenFeature は `core/`・`flagd/`・`flagd-proxy/` を
   出します。2 つのラインのどちらが新しいかという問いは無意味ですが、以前は答えて
   いました — containerd v1.7.28 の運用者に `api/v1.11.0` を「やるべきこと」として
   提示していたのです。いまは他ラインの変更は比較の前に除外され、件数が `note` に
   残ります。**タグは公開されたまま、接頭辞ごと渡してください**
   （`flagd/v0.16.1`、`lts-4081.3.9`）— 素の `v0.16.1` はメインラインとして読まれ、
   flagd のリリースには一致しません。すべてのタグに同じ接頭辞を使うプロジェクト
   （knative-・edge-）はラインが 1 つなので、動作は従来どおりです。
9. **ブリーフィングはブランチを見ます。見なければなりません。** メンテナは
...
10. **`note` — コンポーネントごとの状態マーカー。** 2 種類が現れます。
...
11. **`hint` と `privacy`。** `hint` はこのブリーフィングが推奨ではなく
