...
> 無料ティアは 1 回も完走できず）。無料ティアでは **flash-lite 系を推奨**
> します — 2026-07 時点で full-flash 系の無料ティアはすべて 5 RPM であり、
> エージェント運用に耐えるレート上限を持つのは flash-lite 系のみです
> （60 ラン のキャンペーンで実測）。
...
| `MCP_HTTP_STATELESS` | *(オフ)* | `1` でセッション状態なしの HTTP — `Mcp-Session-Id` の往復がなく、HTTP で 2026-07-28 プロトコルリビジョンを話す唯一のモード（チャート: `statelessHttp`） |
| `MCP_LOG` | `info` | `debug` は呼び出しごとの所要時間を追加。ログは stderr への 1 行 JSON で、**どのレベルでもリクエスト引数を含まない** — エラー文はコピーではなく再構成（チャート: `logLevel`） |
| `MCP_AUDIT` | *(オフ)* | `metadata` はツール呼び出しごとに `event:"audit"` の JSON 1 行(ツール・結果・呼び出し元 clientInfo・引数の*名前*)、`full` は引数の値も(チャート: `auditMode`) |

**監査ストリームについて正直に**: このストリームはあなたのクラスタ内で
生成され、あなたのコレクタに蓄積されます — データが境界を出ないことが
要点です。保持期間や改ざん防止はログ基盤の仕事で、`event=audit` で
ルーティングしてください。正直な境界をひとつ: このサーバーに認証はない
ため、「呼び出し元」とは自己申告の `clientInfo` とトランスポートが示す
ものまでです。記録が証明するのは*どのクライアントがどのツールをどの
引数で呼んだか*までで — どの人間がエージェントに指示したかはエージェント
層だけが知る知識であり、MCP 層の記録には作り出せません。呼び出し元が
送った `traceparent` は記録に `trace_id` として刻まれ、層をつなぐ結合
キーになります。
