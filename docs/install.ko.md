...
설명합니다 — 설치 없이 호스팅 엔드포인트에 연결하는 방법부터, 노트북의
stdio 서버, 쿠버네티스 클러스터 안의 Helm 배포, kagent 통합까지.

**MCP(Model Context Protocol)** 는 AI 에이전트가 외부 도구를 호출할 때 쓰는
표준 프로토콜입니다. ratatosk-mcp는 이 프로토콜로 릴리스 팩트 도구 6개 —
`check_stack`(실행 중인 버전 목록을 받아 업그레이드 경로의 팩트 반환),
`list_facts`(증분 팩트 피드), `facts_by_entity`(CVE·플래그 등 식별자 역조회),
`get_release`(릴리스 하나 전체), `list_releases`(프로젝트별 최신 릴리스 요약),
`list_projects`(추적 프로젝트와 정식 슬러그) — 를 제공하는 서버이고, MCP를
지원하는 클라이언트라면 무엇이든 — Claude Code, Claude Desktop, kagent, 직접
만든 SDK 에이전트 — 연결할 수 있습니다.

## 실행 방식 고르기

| 이런 상황이라면 | 방법 | 섹션 |
| --- | --- | --- |
| 일단 써보고 싶다 — 아무것도 설치하지 않고 | 호스팅 엔드포인트 URL 등록 | [호스팅](#호스팅-엔드포인트) |
...
API 키도 필요 없습니다. 차이는 셋입니다: `check_stack`에 넘긴 실행 중 버전이
어디까지 가는가(설치형은 버전 비교가 사용자 프로세스 안에서 끝나 그 버전이
인프라를 떠나지 않고, 호스팅은 버전이 서버 메모리를 지나가되 어떤 로그에도
기록되지 않습니다), 업스트림 요청 한도를 누구와 나누는가(호스팅은 공유 버킷,
설치형은 자기 IP 몫), 감사 스트림을 남길 수 있는가(설치형만 가능). 한 가지
공통점도 알아두세요: `get_release`처럼 특정 버전을 인자로 받는 도구는 어느
방식에서든 그 버전을 업스트림 요청 경로에 담아 조회합니다 — 조회 대상을
지목하는 값이기 때문입니다. 다만 그 경로도 서버 쪽 로그에는 남지 않습니다:
액세스 로그가 기록 전에 쿼리스트링을 제거하고 `/v1/releases/…`·`/v1/upgrade/…`
경로를 접두사만 남기기 때문입니다. 각 섹션에서 자세히 설명합니다.

## 전제조건

- **호스팅**: 원격 MCP 서버(Streamable HTTP — 스트리밍 방식의 HTTP
  트랜스포트)를 지원하는 클라이언트만 있으면 됩니다. 그 외 준비물 없음.
...
- **클러스터(Helm)**: 쿠버네티스 클러스터와 Helm 3, 그리고 이 레포 클론.
  차트는 별도 차트 저장소나 OCI 레지스트리로 발행되지 않고 레포 안에만
  있습니다.
- **아웃바운드 HTTPS(egress)**: 설치형은 어느 방식이든 서버 프로세스가
  ...
  있습니다. 차단된 채로 실행하면 도구가 오류를 반환하거나 결과가 비어
  옵니다(`check_stack`은 오류 대신 컴포넌트별 `note` 필드에
  `fetch failed: …`를 담아 돌려줍니다) — [트러블슈팅](#트러블슈팅) 참조.
- **kagent 통합**: kagent가 이미 설치된 클러스터(kagent CRD 포함).

## 호스팅 엔드포인트

`https://ratatosk.io/mcp` — 설치 없이 바로 쓰는 원격 MCP 엔드포인트입니다.
Streamable HTTP로 서빙되며 무상태(stateless)입니다: 세션이 없어
...
- **SSE GET은 405**: 무상태 모드는 서버발 알림 스트림(GET으로 여는 SSE)을
  제공하지 않으므로 405 응답이 정상 동작입니다. 브라우저 주소창으로 열면
  안내 페이지(`/docs/mcp`)로 리다이렉트됩니다.
- **공정 사용**: 호스팅 경로는 업스트림 공개 API의 레이트리밋(분당
  60요청/IP) 버킷을 다른 이용자와 함께 씁니다(직접 `/v1`을 호출할 때의
  IP별 한도와는 별개 버킷입니다). 폴링처럼 무거운 사용이 필요하면
  설치형으로 전환하세요.

### 프라이버시 — 호스팅에서 요청 내용이 가는 곳

호스팅에서는 `check_stack` 인자(실행 중인 버전 목록)가 서버 메모리를
지나갑니다. 대신 요청 내용은 어디에도 기록되지 않습니다:
...
- 서버 앞단의 액세스 로그는 쿼리스트링을 제거하고, 슬러그·버전이 경로에
  ...
  서버 로컬에서 크기 기준으로 로테이션됩니다(현재 트래픽 기준 며칠 분량).
- 아래에서 설명할 감사 스트림은 호스팅에서 꺼져 있고 켜지 않습니다.

다만 통제 밖의 경계가 하나 있습니다: CDN(Cloudflare) 구간의 접속
메타데이터는 Cloudflare의 정책을 따르며 이는 ratatosk.io가 통제할 수 없는
영역입니다. 이 경계가 요건에 맞지 않으면 설치형을 쓰세요 — `check_stack`에
넘긴 버전이 프로세스 밖으로 나가지 않습니다.

## 로컬 (stdio)

stdio는 MCP 클라이언트가 서버를 자식 프로세스로 띄우고 표준입출력으로
...
에이전트가 `check_stack`을 호출해 버전 이후의 보안 수정·제거·기본값 변경을
근거 인용과 함께 답하면 성공입니다.

## 클러스터 단독 (Helm)

`MCP_HTTP_ADDR`를 설정하면 같은 바이너리가 `/mcp`에서 Streamable HTTP로
MCP를 서빙합니다(`/healthz` 헬스 체크 포함). 차트는 이를 ClusterIP Service로
배포하고 `MCP_HTTP_ADDR`를 `service.port`(기본 8080)에 맞춰 자동
설정합니다. 클러스터 안의 무엇이든 프로세스를 띄우는 대신 URL로 접속합니다
— SDK로 만든 자체 에이전트, 다른 에이전트 프레임워크, 업그레이드 전에
`check_stack`으로 게이트를 거는 CI 잡까지.
...
프로파일에서 경고 없이 돕니다. 차트 옵션 전체는
[차트 README](../charts/ratatosk-mcp/README.ko.md)에 정리되어 있습니다.
MCP 스펙 2026-07-28 리비전을 쓰는 최신 클라이언트를 HTTP로 받으려면
(`Mcp-Protocol-Version: 2026-07-28` 헤더를 보내는 클라이언트의 경우),
그리고 레플리카를 2개 이상으로 늘릴 계획이면
`--set statelessHttp=true`를 켜세요 — [설정 레퍼런스](#설정-레퍼런스) 참조.
...
> **모델 최소 요건**: 에이전트 런 1회가 내부 모델 호출을 6회 이상 만들고,
> kagent Go ADK는 429(요청 한도 초과) 응답을 재시도하지 않습니다. 따라서
> 분당 약 10요청 이상을 허용하는 모델 티어가 필요하며, 그보다 낮으면 매
> 런이 실패합니다. 예를 들어 Gemini 무료 티어(2026-07 기준)에서는 분당
> 5요청인 full-flash 계열로는 런이 완주되지 않고, flash-lite 계열만
> 에이전트 구동이 가능한 한도를 제공합니다.

## 설정 레퍼런스

서버 동작을 바꾸는 설정은 모두 환경변수이고, 헬름 차트는 각 변수에 대응하는
...
| `MCP_HTTP_ADDR` | *(차트가 `service.port`로 자동 설정)* | *(없음 = stdio)* | 설정 시(예: `:8080`) stdio 대신 `/mcp` Streamable HTTP로 서빙, `/healthz` 포함 |
...
세션 종료 한 줄)과, 업스트림 연결이 아플 때의 경고·오류 줄뿐입니다. 정상
요청은 한 줄도 남기지 않습니다. 참고로 stdio 세션 종료 줄은 두 모양입니다:
클라이언트가 정리하고 끊으면 `"msg":"stdio session ended"`(INFO), 정리 없이
갑자기 끊으면 `"msg":"stdio session ended with error"`(ERROR) — 후자도
장애가 아니라 종료 방식의 기록입니다. 오류가 나도 원래 오류 메시지를 복사하지
않고 — 요청 URL에 실행 중 버전이 들어 있기 때문입니다 — 엔드포인트 패턴과
...
기록입니다. **기본은 꺼짐이고, 꺼진 동안 감사 레코드는 0바이트입니다** —
스트림 자체가 존재하지 않습니다.
...
0.6.1부터 상태 유지(stateful) HTTP 모드 — 차트 기본값 — 로 서빙할 때의
감사 레코드에는 전송 세션 식별자인 `session_id` 필드가 함께 담깁니다(예:
`"session_id":"T3E77BYZFDDA33SIUSORQ365ZL"`) — 동시에 접속한 호출자들을
세션 단위로 구분하는 값입니다. `statelessHttp`(환경변수
`MCP_HTTP_STATELESS`)를 켜면 이름 붙일 세션이 없으므로 `session_id`도
클라이언트가 보고한 `clientInfo`도 레코드에서 빠지고, 그 모드에서 호출
단위를 이어 주는 키는 아래의 `trace_id`뿐입니다 — 감사 스트림에서
호출자를 구분해야 한다면 상태 유지 모드로 돌리거나 호출자가
`traceparent`를 보내게 하세요. 위 예시 같은 stdio 레코드에도
`session_id`는 없습니다 — 프로세스 하나에 호출자가 하나뿐이라 구분할 것이
없기 때문입니다.
...
`audit`인 레코드를 별도 싱크로 라우팅하면 운영 로그와 다른 보존 정책을 줄
수 있습니다.

한계도 알아두세요: 이 서버에는 인증이 없으므로 기록의 "호출자"는
클라이언트가 자가 보고한 `clientInfo`와 전송 계층이 보여주는 정보까지입니다.
어떤 사람이 에이전트에게 그 호출을 시켰는지는 에이전트 계층의 기록에서
찾아야 합니다.
...
업스트림까지 하나의 `trace_id`로 이을 수 있습니다. 위 감사 기록의 계층
한계를 잇는 조인 키가 바로 이것입니다. 형식이 잘못된 값은 버려지고
...
| kagent 에이전트가 매 런 실패 | 에이전트 로그의 429 | 모델 티어의 요청 한도 부족 — [kagent와 함께](#kagent와-함께)의 모델 최소 요건 참조 |
...
- **Helm**: 차트는 차트 저장소로 발행되지 않고 레포 클론으로만 설치합니다.
  특정 버전에 고정하려면 클론 후 릴리스 태그를 체크아웃하세요:

  ```bash
  git -C ratatosk-mcp checkout v0.6.1
  ```

  이미지 태그는 기본으로 차트의 `appVersion`을 따르고, `image.tag` 값으로
  덮어쓸 수 있습니다.
...
- 도구 6개의 상세 설명: [README의 도구 표](../README.ko.md#도구)
