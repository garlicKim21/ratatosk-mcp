...
7. **병합 규칙 둘, 실측 응답에 다 보입니다.** 갈래를 가로질러 `matter_key`가
   같은 변경들은 가장 이른 수정으로 접히고, 그 사안을 실은 **다른 모든
   릴리스가**(반환 창 안이든 밖이든) `same_matter_also_addressed_in`에
   실립니다. 운영자는 한 번 올리므로 그 사안을 닫는 가장 가까운 릴리스가
   실행 가능한 답이기 때문입니다. 이와 별개로, **한 릴리스 안에서** 인용문이
...
9. **브리핑은 갈래를 압니다. 그래야만 합니다.** 메인테이너는 한 수정을
   ...
   됩니다 — 자기 갈래가 한 릴리스 전에 닫은 일을 하라는 뜻이 되죠. 그래서
   **실행 버전의 갈래에서 실행 버전 이하에 이미 고쳐진** 사안은 제외하고,
   몇 건을 뺐는지 `note`에 적습니다. 갈래가 같아야 한다는 조건이 이걸
...
제한은 두 가지이고 단위가 다릅니다. 공개 API는 **IP당 분당 1200회**를
허용합니다 — 직접 호스팅하는 서버가 쓰는 몫이 이것입니다. 호스팅
엔드포인트는 여기에 더해 **호출자당 분당 도구 호출 60회**를 허용하고,
전체가 나눠 쓰는 몫이 아니라 호출자별로 셉니다.

도구 호출 1회는 요청 1회가 아닙니다. 여기 있는 도구는 `check_stack`만
빼면 전부 상류 요청 1회로 끝나고, `check_stack`은 컴포넌트당 1회(변경이
없는 것으로 드러난 컴포넌트마다 1회 더)를 씁니다. 컴포넌트 15개짜리
`check_stack`은 도구 호출 1회에 요청 약 15회인 셈인데, 그래도 `list_changes`를
15번 따로 부르는 것보다 훨씬 쌉니다. 프로젝트마다 폴링하는 대신 스택 질문을
한 번에 묶으라는 이유가 그것입니다. 많이 쓰거나 자동화한다면 직접 호스팅으로
옮기세요.
